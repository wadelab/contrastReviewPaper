...
where $C_\mathrm{mask}$ reflects the contrast of the mask component at a distinct frequency from that of the target. The effect of this extra term is to reduce the target response (see right panel of @fig-twoinputs). For a linear contrast axis, the contrast response function becomes shallower, whereas on a logarithmic contrast axis it maintains its steepness and shifts to the right. Suppressive effects of this kind have been obtained using SSVEP with a variety of different types of mask, including orthogonal overlaid masks, surround masks, and dichoptic masks (Burr and Morrone 1987; Ross and Speed 1991, Candy, Norcia, Busse, [@salelkar2020interaction] [for a meta-analysis see @cunningham2017NonLinear, @Baker2021].
...
Attention exerts a profound influence on visual performance: For example, allowing people to attend covertly to a spatial location for example, improves their performance on a target detection task significantly (Posner, https://pubmed.ncbi.nlm.nih.gov/11934448/, https://pubmed.ncbi.nlm.nih.gov/10788636/,  (Morrone, Denti, & Spinelli,
2004; Pestelli, Ling, & Carrasco, 2009) , [@bashinski1980enhancement]). 

In principle, this enhancement may be driven both by modulation of the underlying signal or noise characteristics, or by additional decision-theoretic factors such as reduction in spatial uncertainty (https://jov.arvojournals.org/article.aspx?articleid=2121966, https://jov.arvojournals.org/article.aspx?articleid=2192826). 

Early experiments with non-human primates showed little evidence for attentionally-driven changes in neuronal spike rates (Luck,
Chelazzi, Hillyard, & Desimone, 1997; Marcus & van
Essen, 2002; McAdams & Maunsell, 1999, Mehta, Ulbert, and
Schroeder (2000)), but with the advent of spatially-resolved human brain imaging methods in the late 90s it became apparent that spatial attention was linked to frank changes in both fMRI bold responses (Tootell, 98: (Brefczynski &
...
1998),) and EEG signals (https://pubmed.ncbi.nlm.nih.gov/16221931/), . 

Electrophysiological and psychophysical measurements of attentional effects on both luminance and chromatic contrast strongly implicated gain control an an underlying mechanism [@dirusso99Attention, @DiRusso2001GainControl, @lu1998external] and demonstrated that these effects differed between chromatic and achromatic pathways [@dirusso1999mcpc].  In the late 2000s a comprehensive theoretical model for attentional modulation was developed that framed it as a gain control computation (Reynolds and Heeger 2009, Boynton Vis Res 2009). This framework has proven to be influential - explaining a wide range of phenomena from the earlier literature and demonstrating subtle interactions between the size of the attentional 'spotlight' and the stimulus configuration which rationalise many apparent contradictions in the literature. Subsequent papers   (effect of stimulus configuration critical). Then... talk about Lauriten et al, Verghese (2 papers) maybe mention Crognale / Highsmith, Wang+Wade and Jasna's paper from 2018. https://www.sciencedirect.com/science/article/abs/pii/S1053811918303860) Probably lots more Carasco ...



# Brain-computer interfaces

One widespread recent application of the SSVEP technique is in the design of brain-computer interfaces, which seek to control some aspect of a computer using neural signals. The high SNR and frequency resolution of SSVEPs make them an ideal candidate for this approach. Typical studies may involve presenting an array of stimuli at different flicker frequencies, and having the participant select one either by overt attention (i.e. shifting fixation to foveate the selected stimulus) or covert attention (i.e. deploying attention to one stimulus whilst keeping fixated). EEG signals are highly sensitive to both visual field position [@Ales2010] and attentional state [@Verghese2012; @Lauritzen2010], so the response to the selected stimulus will typically increase relative to the others, allowing it to be identified by an on-line algorithm.

Probably a summary of BCIs here.
