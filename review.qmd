...
Previous figures showed how a single-input SSVEP might generate a single set of peaks in the frequency domain response which, in turn, can be used as a read-out of neuronal activity. In @fig-plotspectra, we show the effect of adding a second contrast component: the two components interact generating nonlinear intermodulation terms at sums and differences of the input frequencies (F1, F2). The nature of the interaction --- and therefore the pattern of intermodulation terms ---  is determined by the computations happening as the contrast signal moves from retina to cortex [@Regan1988]

First, we implement a simple non-linear transducer with a single input to illustrate the typical sigmoidal shape of a contrast response function (@fig-contrastresponsefunction).

Next, we show that adding a second component which contributes to the gain pool of the first, results in a complex pattern of intermodulation terms (@fig-plotspectra).
...
One of the simplest nonlinearities is the function that describes a cell's response to different levels of contrast. In @fig-contrastresponsefunction this is modeled as described above by a hyperbolic ratio function resulting in a saturating non-linearity:

\begin{equation}
R_{\mathrm{max}} = \frac{C_{\mathrm{in}}^n}{C_{50}^n + C_{\mathrm{in}}^n},
\end{equation}

where ${R_{\mathrm{max}}}$ describes the maximum response level, $C_{\mathrm{in}}$ is the input contrast, $C_{50}$ is the 'semi-saturation constant' (the point at which the response is at half-maximum) and *n* controls the steepness of the curve.

The hyperbolic ratio function is monotonic but the CRF resulting from measuring the amplitude of the 2F component contains a slight roll-off at high input contrasts. This results from the distortion of the input sine waves at high contrast due to a combination of the full-wave rectification and saturating non-linearity. Power at other harmonics increases, and the total power generated by the input is monotonic. This roll-off is often seen in experimental data and has been referred to as 'supersaturation' [@Tyler1985; @Peirce2007].

To illustrate the effect of contrast gain control [@Heeger1992], we include a second component that contributes to the gain pool of the first. At a single pair of (matched) contrast levels, we see a complex pattern of intermodulation terms in the FT (@fig-intermodulation).
...
The complexity of even a simple simulation of the frequency-domain signal arising from non-linear interactions is intriguing. Presumably, the signal measured from early visual cortex contains is the result of a cascade of nonlinear retinal and cortical operations up to that point. It therefore contains a 'signature' of the computational nature, order and parameters of those operations - including the shape of the transducer functions and the computations involved in signal combination. In principle, that information could be recovered from the SSVEP signal. 

This possibility was recognised in the early days of the technique [@Regan1988]. Although characterising the complete set of computations along entire processing pathway is problematic, careful parametric variation of the input stimuli does allow us to fit models of early visual gain control using SSVEP data. ... MORE HERE - Although this idea was suggested as early as 1988 (probably earlier) not many people seem to have done it much. @Candy2001; @Busse2009; @Tsai2012; @Baker2017
...
Because SSVEP responses at detection threshold are very small, estimating threshold is achieved by measuring the contrast response function at relatively high levels, and extrapolating back along the function (either contrast vs response measured at a constant spatial frequency or spatial frequency vs response at a constant contrast level) to estimate its intercept with the x-axis (see @fig-sweepvep). This contrast level was shown to correspond approximately with psychophysically measured detection thresholds [@Norcia1986]. 
...
Alex - it would be good to add some CRFs from @Afsari2014 to the figure. Is it possible to extract either the fitted parameter values or the raw data for some control flies and some mutants? Whatever you think is most compelling.
...
# Assuming Parkinson's parameters are similar to those given for epilepsy/autism,
# but with specifics for Parkinson's. We'll use the epilepsy parameters as a placeholder.

# Note: Replace epilepsyparams with Parkinson's specific parameters as necessary.
respParkControls = epilepsyparams[0,0] * ((finecontrasts/100)**epilepsyparams[0,2] /
                      (epilepsyparams[0,1]**epilepsyparams[0,2] + (finecontrasts/100)**epilepsyparams[0,2])) + epilepsyparams[0,3]

respParkPatients = epilepsyparams[1,0] * ((finecontrasts/100)**epilepsyparams[1,2] /
                      (epilepsyparams[1,1]**epilepsyparams[1,2] + (finecontrasts/100)**epilepsyparams[1,2])) + epilepsyparams[1,3]

plt.subplot(2, 2, 4)  # Fourth plot for Parkinson's
plt.plot(finecontrastsdB, respParkPatients, lw=4, color='red', label='Parkinson\'s patients')
plt.plot(finecontrastsdB, respParkControls, lw=4, color='black', linestyle='--', label='Control participants')
plt.xticks(ticklocsx, ticklabelsx)
plt.yticks(np.arange(0, 7, 2))
plt.xlabel('Target contrast (%)')
plt.ylabel('SSVEP response')
...
pdf('Figures/clinicalplot.pdf',width=12,height=10)
par(mfrow=c(2,2))
...
# fits to data from Tsai et al. (2011)
...
respEPIcontrols <- epilepsyparams[1,1] * ((finecontrasts/100)^epilepsyparams[1,3]/(epilepsyparams[1,2]^epilepsyparams[1,3] + (finecontrasts/100)^epilepsyparams[1,3])) + epilepsyparams[1,4]

respEPIpatients <- epilepsyparams[2,1] * ((finecontrasts/100)^epilepsyparams[2,3]/(epilepsyparams[2,2]^epilepsyparams[2,3] + (finecontrasts/100)^epilepsyparams[2,3])) + epilepsyparams[2,4]
...
title(main='(a) Epilepsy',cex.main=2)

lines(finecontrastsdB,respEPIpatients,lwd=4,col='blue')
lines(finecontrastsdB,respEPIcontrols,lwd=4,lty=2,col='black')

legend(-18,6,c('Epilepsy patients','Control participants'),lwd=4,lty=c(1,2),col=c('blue','black'),box.lwd=2)


# fits to data from Baker et al.. (2015)
amblyparams <- matrix(0,nrow=2,ncol=4)
amblyparams[1,] <- c(0.39,37.2,1.4,0.22)  # amblyopic eye
amblyparams[2,] <- c(2.33,111.69,1.4,0.22)  # good eye

respambly <- amblyparams[1,1] * (finecontrasts^amblyparams[1,3]/(amblyparams[1,2] + finecontrasts^amblyparams[2,3])) + amblyparams[1,4]

respfellow <- amblyparams[2,1] * (finecontrasts^amblyparams[2,3]/(amblyparams[2,2] + finecontrasts^amblyparams[2,3])) + amblyparams[2,4]
...
title(xlab="Target contrast (%)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)   
title(ylab="SSVEP response", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)
title(main='(b) Amblyopia',cex.main=2)

lines(finecontrastsdB,respambly,lwd=4,col='red')
lines(finecontrastsdB,respfellow,lwd=4,lty=2,col='black')

legend(-18,2,c('Amblyopic eye','Fellow eye'),lwd=4,lty=c(1,2),col=c('red','black'),box.lwd=2)


# fits to data from Vilidaite et al (2018)
...
respASD <- (autismparams[1,1]*(finecontrasts^autismparams[1,4])/(autismparams[1,3] + finecontrasts^autismparams[1,4])) + autismparams[1,2]
respCON <- (autismparams[2,1]*(finecontrasts^autismparams[2,4])/(autismparams[2,3] + finecontrasts^autismparams[2,4])) + autismparams[2,2]
...
title(xlab="Target contrast (%)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)   
title(ylab="SSVEP response", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)
title(main='(c) Autism',cex.main=2)

lines(finecontrastsdB,respASD,lwd=4,col='darkgreen')
lines(finecontrastsdB,respCON,lwd=4,lty=2,col='black')

legend(-18,1.5,c('ASD children','Control children'),lwd=4,lty=c(1,2),col=c('darkgreen','black'),box.lwd=2)
...
title(xlab="Target contrast (%)", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)   
title(ylab="SSVEP response", col.lab=rgb(0,0,0), line=1.5, cex.lab=1.5)
title(main="(d) Parkinson's",cex.main=2)



dev.off()

``` commented out R because of the weird reticulate bug...--->


![caption.](Figures/clinicalplot.pdf){#fig-plotclinical}
